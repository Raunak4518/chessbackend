...
export class DailyGamesService {
  ...
  async handleTimeouts() {
    ...
    for (const game of timedOutGames) {
      ...
      this.logger.log(`Game ${game.id} timed out. Winner: ${isWhiteTurn ? 'BLACK' : 'WHITE'}`);
    }
  }

  async createDailyGame(whitePlayerId: string, blackPlayerId: string, daysPerMove: number) {
  ...
  async getMyDailyGames(userId: string) {
    return this.prisma.game.findMany({
      ...
      include: {
        whitePlayer: { select: { id: true, name: true, rating: true, ratingDaily: true, image: true } },
        blackPlayer: { select: { id: true, name: true, rating: true, ratingDaily: true, image: true } }
      },
  ...
  async makeMove(userId: string, gameId: string, from: string, to: string) {
    ...
    if (!game) throw new NotFoundException('Game not found');
    if (game.status === 'COMPLETED' || game.status === 'ABANDONED') throw new BadRequestException('Game is over');
