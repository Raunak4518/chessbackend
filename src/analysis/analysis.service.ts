...
export class AnalysisService {
  ...
  async analyzeGame(gameId: string) {
    ...
    await this.prisma.game.update({
      where: { id: gameId },
      data: { analysis: finalAnalysis as any },
    });
  ...
  async analyzePgn(pgn: string) {
      ...
      const promises = chunk.map(async (pos) => {
        try {
          ...
          return {
            ...
            centipawns: parseInt(data.centipawns) || (data.eval * 100) || 0,
            depth: 15
          };
        } catch (e) {
          this.logger.error(`Error fetching analysis for FEN ${pos.fen}: ${e.message}`);
    ...
    for (let i = 1; i < analysisData.length; i++) {
      ...
      if (i > 10) { // After opening (very rough heuristic for Book moves)
        const prevCp = prev.mate ? (prev.mate > 0 ? 10000 : -10000) : prev.centipawns;
        const currCp = curr.mate ? (curr.mate > 0 ? 10000 : -10000) : curr.centipawns;
        ...
        if (probLoss > 20 || evalChange <= -300) {
          if (prev.mate && !curr.mate) {
            ...
            classification = 'Blunder';
            explanation = 'A critical error that severely worsens your position.';
            if (evalChange <= -300) explanation = 'Blundered significant material or positional advantage.';
          }
        ...
        } else if (probLoss < -10 && currCp * (curr.color === 'w' ? 1 : -1) < 0) {
           classification = 'Miss';
        ...
        } else if (evalChange > 0 && currCp * (curr.color === 'w' ? 1 : -1) > 300) {
        ...
        if (evalChange > 200 && curr.move.includes('x') === false && prevCp < currCp) {
      ...
      else if (classification === 'Best Move' || classification === 'Great' || classification === 'Brilliant' || classification === 'Book') moveAccuracy = 100;
